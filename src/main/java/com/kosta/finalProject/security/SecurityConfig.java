...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests() // HttpServletRequest에 따라 접근(access)을 제한
				.antMatchers("/**","/blog/**", "/post/**", "/post_reply/**","/user/**").permitAll() //   누구나 접근 허용
				//.antMatchers("/**").hasRole("1") // /admin으로 시작하는 경로는  ADMIN롤을 가진 사용자만  접근 가능(자동으로 ROLE_가 삽입)
				//.antMatchers("/manager/**").hasRole("MANAGER")
				//.anyRequest().authenticated() // 나머지 요청들은 권한의 종류에 상관 없이 권한이 있어야 접근 가능
				.and() //한문장으로 계속 주기위해 and를 씀. 즉 어떤 기능이 끝나고 다른 기능을 이어줄 때 사용함.
				.formLogin() // form 기반으로 인증을 하도록 한다. 로그인 정보는 기본적으로 HttpSession을 이용
				.loginPage("/index") // 로그인 페이지 링크 .... post의 이름이 같다면 loginProcessingUrl생략 
